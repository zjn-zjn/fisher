...
type TradeState struct {
	...
	FromWallets WalletList             `json:"from_wallets" gorm:"column:from_wallets;"`                 // 扣款钱包信息列表
...
func AssembleTradeState(fromWallets, toWallets []*TradeWalletItem, tradeId int64, tradeScene basic.TradeScene, status basic.TradeStateStatus, coinType basic.CoinType, comment string) *TradeState {
	md := &TradeState{
		TradeId:    tradeId,
	...
	if fromWallets == nil {
		md.FromWallets = make(WalletList, 0)
	} else {
		md.FromWallets = fromWallets
	}
	if toWallets == nil {
