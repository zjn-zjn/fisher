...
func RollbackTrade(ctx context.Context, req *model.RollbackTradeReq) error {
	...
	err := dao.WalletDBTX(ctx, func(ctx context.Context, db *gorm.DB) error {
		...
		if state == nil {
			state = model.AssembleTradeState(nil, nil, req.TradeId, req.TradeScene, basic.TradeStateStatusRollbackDone, 0, "empty rollback")
	...
	for _, v := range state.ToWallets {
		v := v
		err = dao.DeductionWallet(ctx, v.WalletId, state.TradeId, v.Amount, state.CoinType, req.TradeScene, basic.TradeRecordStatusRollback, v.ChangeType, fmt.Sprintf("rollback %s", v.Comment))
		if err != nil {
			return err
		}
	}
	//对加的钱包进行扣减
	for _, v := range state.FromWallets {
		v := v
		err = dao.IncreaseWallet(ctx, v.WalletId, state.TradeId, v.Amount, state.CoinType, req.TradeScene, basic.TradeRecordStatusRollback, v.ChangeType, fmt.Sprintf("rollback %s", v.Comment))
		if err != nil {
			return err
		}
	}
